...
/// Permute metric index even we have both Dirac and mertic ones
	Matrix<_Tp> DiracPermute(int _mu, int _nu)
	{
		Matrix<_Tp> ret;
		ret=(*this);
		for(int i = 0; i < 4; i++){
		for(int j = 0; j < 4; j++) ret.Element(i,j) = this->Element(i,j).Permute(_mu,_nu);
		}
		return ret;
	}
