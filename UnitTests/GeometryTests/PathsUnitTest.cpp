...
// SinglePointTest
t_cmplx C_PathsUnitTest::SinglePointTest(double t_paramtr, int PathType) {
	t_cmplx ParabolaApex = t_cmplx(0.0, 1.0);
	t_cmplx AxisShiftY = t_cmplx(1.0, 0.0);
...
t_cmplxArray1D C_PathsUnitTest::getPathOnVector(t_cmplxArray1D SamplePoints,
		Geometry::C_Path * PathInstance) {
	return PathInstance->getPathOnVector(SamplePoints);
}

t_cmplx C_PathsUnitTest::getPathAtPoint(t_cmplx t_paramtr,
		Geometry::C_Path * PathInstance) {
	return PathInstance->getPathAt(t_paramtr);
}

t_cmplxArray1D C_PathsUnitTest::SamplePointsForTest(int NumTestPoints) {
