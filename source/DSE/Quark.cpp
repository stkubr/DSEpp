...
C_Quark::~C_Quark() {
	...
	delete Integrator_momentum_cutoff;
	delete Integrator_angle_Z;
	delete Integrator_cauchy;
...
void C_Quark::readQuarkParameters(string &_ParamPath) {
 ...
 */
void C_Quark::setToInitialState() {
	A_renorm = 1.0;
	B_renorm = 0.0;
	B_mu = 0.0;
	Z2 = 1.0;
	...
	CauchyIntegratonWeight_lambda = [](t_cmplx path_var, t_cmplx coordinate) {
		return 1.0 / (path_var - coordinate);
	};
 ...
 * the contour is a parabola along real axis with an apex (-params.M2_contour,0), cutted at "params.LimUk" in UV.
 */
void C_Quark::setContour() {
	t_cmplx last_point;
	t_cmplxArray2D tempContour;
	last_point = zz_radial[zz_radial.size() - 1] * zz_radial[zz_radial.size() - 1] - params.M2_contour / 4.0 + params.LimUk * params.LimDk;
	Geometry::C_ParabolaContour contour(ii * sqrt(params.M2_contour) / 2.0,
			ii * sqrt(params.M2_contour),
			last_point);
	contour.setParabolaContour(zz_radial, w_radial, zz_cutoff, w_cutoff);
 ...
 * equation for the grid is (p - k)^2, where p^2 in the contour and k is integration momentum in DSE
 */
		...
		for (int j = 0; j < params.num_prop_steps; j++) {
			for (int k = 0; k < params.num_angle_steps; k++) {
				Memory->S_grid[0][i][params.num_angle_steps * j + k] =
						...
						/* k^2 */    + zz_radial[j + 1] * zz_radial[j + 1]
						/* 2(pk) */  - 2.0 * sqrt(Memory->S_cont[0][i] * zz_radial[j + 1] * zz_radial[j + 1]) * zz_angle[k + 1];
			}
		}
	}
	//draw grid
...
void C_Quark::setInitialAandB() {
	auto initA = [](t_cmplx z) -> t_cmplx {
		return 1.0 + exp(-z * z);
	};
	auto initB = [](t_cmplx z) -> t_cmplx {
		return exp(-z * z);
	};
...
/**
 * Calculation A,B,M,Sigma_V,Sigma_S at point inside a contour
 */
t_cmplxArray1D C_Quark::PropagatorAtPoint(t_cmplx q) {
...
/**
 * Check convergence
 * calculates the difference between current checksum and previous iteration
 */
...
/**
 * Dress Propagator
 * meaning to solve the quark DSE iteratively
 */
...
/**
 * Calculates sum A * B functions at 100 points.
 */
